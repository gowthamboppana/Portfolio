...
function Hero() {
  return (
    <section id="home" className="flex min-h-[85vh] scroll-mt-24 flex-col justify-center py-16 sm:py-24" aria-labelledby="hero-title">
      <p className="animate-fadeUp text-sm font-medium uppercase tracking-[0.2em] text-accent">
        {heroData.greeting}
      </p>
      ...
      <ul className="mt-5 flex animate-fadeUp flex-wrap gap-2" aria-label="Core technologies">
        {heroData.highlights.map((item) => (
          <li
            key={item}
            className="rounded-full border border-border bg-card px-4 py-1.5 text-sm font-medium tracking-wide text-muted transition hover:border-accent hover:text-accent"
          >
            {item}
          </li>
        ))}
      ...
      <div className="mt-8 flex animate-fadeUp items-center gap-3">
        ...
        <SocialLink href={`mailto:${socialLinks.email}`} label="Send email">
          <svg viewBox="0 0 24 24" className="h-5 w-5 fill-current" aria-hidden="true">
